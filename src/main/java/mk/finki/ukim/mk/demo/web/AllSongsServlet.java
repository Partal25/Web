...
public class AllSongsServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        Artist a = null;
        try {
            a = artistService.findById(Long.valueOf(artistId));
        } catch (NotFoundException e) {
            try {
                throw new NotFoundException("Artist with artistId does not exist!");
            } catch (NotFoundException ex) {
                throw new RuntimeException(ex);
            }
        }
        List<Song> songs = songService.listSongs().stream().filter(s -> s.getPerformers().contains(artistId)).toList();
